...
def _aseprite_export_sheet_impl(ctx):
    ...
    ctx.actions.run(
        outputs = [ctx.outputs.out_sheet, ctx.outputs.out_json],
        inputs = ctx.files.srcs,
        executable = "aseprite",
        arguments = [
            "--batch",
            "--sheet=" + ctx.outputs.out_sheet.path,
            "--data=" + ctx.outputs.out_json.path,
            "--format=json-array",
            "--filename-format={title}:{tag}:{tagframe}",
            "--sheet-type=packed",
    ...
    return DefaultInfo(files = depset([
        ctx.outputs.out_sheet,
        ctx.outputs.out_json,
    ]))
...
aseprite_export_sheet = rule(
    ...
    attrs = {
        ...
        "out_sheet": attr.output(mandatory = True, doc = "output PNG file"),
        "out_json": attr.output(
            mandatory = True,
            doc = "output metadata JSON file",
        ),
    },
)
